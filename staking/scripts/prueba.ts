...
import { deployData, full, saveDeployData, getStakeConnection } from "./common";
...
const main = async () => {
  ...
  // console.log("Loocking for stake account: ", deployData.stakeAccountPositions);
  const [stakeAcc] = await stakeConnection.getStakeAccounts(publisher.publicKey);
  // console.log("StakeConnection:", stakeConnection);
  // console.log("Stake: ", deployData.stakeAccountPositions);
  // let stakeAcc = await stakeConnection.loadStakeAccount(new PublicKey(deployData.stakeAccountPositions));
  ...
  //   new PublicKey(deployData.stakeAccountPositions)
  // );
...
;
  ...
  // .delegate(new BN(100_000_000_000))
  // .accounts({
